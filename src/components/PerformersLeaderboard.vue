<template>
  <div class="leaderboard performers-leaderboard">
    <div class="title-container">
      <h2 class="leaderboard-title animate-glitch">⭐ TOP PERFORMERS</h2>
      <p class="leaderboard-description">Highest season best scores in 2025.</p>
      ...
      >
        ...
        <div class="athlete-info">
          ...
          <div class="athlete-stats">
            <span class="stat-value highlight">{{ athlete.seasonBest }}</span>
            <span class="stat-label">SCORE</span>
          </div>
          <div class="discipline-label">{{ athlete.discipline }}</div>
        </div>
...
const athletes = ref<PerformerAthlete[]>([])
...
onMounted(async () => {
  ...
  try {
    const response = await fetch('/data/performers_stats_2025.json')
    console.log('Performers leaderboard fetch response:', response.status, response.ok)
    if (response.ok) {
      const data = await response.json()
      console.log('Performers data loaded:', data)
      athletes.value = data
    } else {
      console.error('Failed to fetch performers data:', response.status)
    }
  } catch (error) {
    console.error('Error loading performers leaderboard:', error)
  } finally {
    isLoading.value = false
  }
})

function handleAthleteClick(athlete: PerformerAthlete) {
